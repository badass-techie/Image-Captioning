...
from tensorflow.keras import Model, Sequential, layers, initializers, applications
import numpy as np
...
def positional_encoding_2d(row, col, d_model):
    ...
    angles_row = get_angles(row_pos, np.arange(d_model // 2)[np.newaxis, :], d_model // 2)
    angles_col = get_angles(col_pos, np.arange(d_model // 2)[np.newaxis, :], d_model // 2)

    # apply sin to even indices in the array; 2i
    angles_row[:, 0::2] = np.sin(angles_row[:, 0::2])
    angles_row[:, 1::2] = np.cos(angles_row[:, 1::2])

    # apply cos to odd indices in the array; 2i+1
    angles_col[:, 0::2] = np.sin(angles_col[:, 0::2])
    angles_col[:, 1::2] = np.cos(angles_col[:, 1::2])

    pos_encoding = np.concatenate([angles_row, angles_col], axis=1)[np.newaxis, ...]
...
class Encoder(layers.Layer):
    ...
    def __init__(self, num_layers, d_model, num_heads, dff, row, col, rate=0.1, name="encoder"):
        super(Encoder, self).__init__(name=name)

        self.d_model = d_model
        self.num_layers = num_layers

        self.embedding = layers.Dense(self.d_model, activation='relu')   # we use a dense layer here because the image features are
                                                                         # already shaped as embeddings - (batch_size, len_features, embedding_dim),
                                                                         # so we only need to adjust their embedding dimension to match d_model
        self.pos_encoding = positional_encoding_2d(row, col, self.d_model)
...
class Decoder(layers.Layer):
    ...
    def __init__(self, num_layers, d_model, num_heads, dff, target_vocab_size,
                 maximum_position_encoding, embedding_matrix, trainable_embeddings, rate=0.1, name="decoder"):
...
class Transformer(Model):
    """
        ...
        Params:
            ...
            target_vocab_size : int
                Size of the target vocabulary
            d_row : int
                Row dimension of the input sequence
            d_col : int
                Column dimension of the input sequence
            pe_target : int
    ...
    def __init__(self, num_layers, d_model, num_heads, dff, target_vocab_size, d_row, d_col, pe_target,
                 embedding_matrix, trainable_embeddings=False, dropout_rate=0.1):
        super(Transformer, self).__init__()
        self.encoder = Encoder(num_layers, d_model, num_heads, dff, d_row, d_col, dropout_rate)
        self.decoder = Decoder(num_layers, d_model, num_heads, dff, target_vocab_size, pe_target, embedding_matrix, trainable_embeddings, dropout_rate)
    ...
    def create_masks(self, inp, target):
        ...
        enc_padding_mask = None # we do not need a padding mask for the encoder input,
                                # because the encoder's input is the image's features,
                                # which are always the same length and never have any padding

        # Used in the 2nd attention block in the decoder.
        # This padding mask is used to mask the encoder outputs.
        dec_padding_mask = None # same reason as above
...
class Captioner(Model):
    ...
    def __init__(self, transformer, max_length):
        ...
        self.feature_extractor = applications.EfficientNetB0(include_top=False, weights='imagenet')
    ...
    def call(self, inputs):
        ...
        img = applications.efficientnet.preprocess_input(inputs)
        features = self.feature_extractor(img)
        features = tf.reshape(features, [features.shape[0], -1, features.shape[-1]])  # None,7,7,960 -> None,49,960
        ...
        def generate_next_token(tar):
            ...
            out, _ = self.transformer([features, tar], training=False) # shape: [1, seq_length, vocab_size]
